...
from regressor import map_features

features_list = list(map_features.keys())
...
def create_feature(text, words, sent_text):
    ...
    stylometry={**count_letter_freq(text), **stylometry}
...
def features_array_from_string(text, columns):

    sent_text = sent_tokenize(text)
            
    words=RemoveSpecialCHs(text)
    
    stylo_dict = create_feature(text, words, sent_text)

    return np.array([stylo_dict[k] if k in stylo_dict.keys() else 0 for k in columns])